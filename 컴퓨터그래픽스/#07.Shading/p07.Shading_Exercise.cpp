...
readMesh(const char* filename)
{
	...
	for (int i = 0; i < nFaces; i++)
	{
		...
		fnormal[i] = v;
		//add it th the normal vector of each vertex
		vnormal[face[0][i]] += v;
		vnormal[face[1][i]] += v;
		vnormal[face[2][i]] += v;
	}
...
drawFlatMesh()
{
	...
	for (int i = 0; i < nFaces; i++)
	{
		...
		glNormal3fv(value_ptr(fnormal[i]));
		glVertex3f(fnormal[i].x / 9.5f + (vertex[face[0][i]].x + vertex[face[1][i]].x + vertex[face[2][i]].x) / 3.0f
			, fnormal[i].y / 9.5f + (vertex[face[0][i]].y + vertex[face[1][i]].y + vertex[face[2][i]].y) / 3.0f
			, fnormal[i].z / 9.5f + (vertex[face[0][i]].z + vertex[face[1][i]].z + vertex[face[2][i]].z) / 3.0f
		);

		glVertex3f((vertex[face[0][i]].x + vertex[face[1][i]].x + vertex[face[2][i]].x
			) / 3.0f, (vertex[face[0][i]].y + vertex[face[1][i]].y + vertex[face[2][i]].y
				) / 3.0f, (vertex[face[0][i]].z + vertex[face[1][i]].z + vertex[face[2][i]].z
					) / 3.0f);
...
drawSmoothMesh()
{
	...
	for (int i = 0; i < nFaces; i++)
		for (int j = 0; j < 3; j++)
		{
			...
			glNormal3fv(value_ptr(vnormal[face[j][i]]));
			glVertex3f(vnormal[i].x / 9.5f + vertex[face[j][i]].x
				, vnormal[i].y / 9.5f + vertex[face[j][i]].y
				, vnormal[i].z / 9.5f + vertex[face[j][i]].z
			);

			glVertex3f(vertex[face[j][i]].x
				, vertex[face[j][i]].y
				, vertex[face[j][i]].z
			);
